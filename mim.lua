  ...
  - step: 1
    ...
    substeps:
      ...
      - name: marketplace_search
        ...
        serp_analysis: |
          === ЭТАП 0: ПРЕДВАРИТЕЛЬНЫЙ АНАЛИЗ ВСЕЙ ПОИСКОВОЙ ВЫДАЧИ ===
          
          ПЕРЕД любыми физическими переходами агент ОБЯЗАН проанализировать ВСЮ первую страницу выдачи Google:
          
          ШАГ 0.1: СБОР РЕЗУЛЬТАТОВ ВЫДАЧИ
            - Выполнить browser_snapshot() на странице результатов Google
            - Извлечь ВСЕ органические результаты (обычно 8-10 ссылок)
            - Исключить рекламные блоки, "похожие запросы", блоки "Люди также ищут"
          
          ШАГ 0.2: АНАЛИЗ КАЖДОГО РЕЗУЛЬТАТА ПО КРИТЕРИЯМ ЗАДАЧИ
            Для КАЖДОЙ ссылки из выдачи оценить по сниппету/заголовку/URL:
            
            a) Релевантность товару (колонка A):
               - Заголовок содержит название/модель/артикул искомого товара? (+30%)
               - Сниппет описывает конкретный товар, а не категорию? (+20%)
               - URL ведёт на /product/, /tovar/, /item/, /p/ ? (+20%)
               - URL НЕ ведёт на /catalog/, /category/, /search/, /?q= ? (+10%)
            
            b) Соответствие критериям из requirements_checklist:
               - Бренд в заголовке/сниппете НЕ из brand_blacklist (J)? Если из списка → пометить "⛔ запрещённый бренд"
               - Категория НЕ из category_blacklist (K)? Если из списка → пометить "⛔ запрещённая категория"
               - Нет признаков нестандартного товара (H): "ОЛ", "ТЗ", "Черт.", "по чертежу"?
               - Нет признаков снятия с производства (I): "discontinued", "снят с производства"?
               - Нет признаков "под заказ" (L): "под заказ", "по запросу", "нет в наличии"?
            
            c) Приоритет источника:
               - Специализированный магазин (krepeg.ru, vseinstrumenti.ru и т.д.) → высокий приоритет
               - Интернет-магазин с ценой в сниппете → средний приоритет
               - Информационный сайт, форум → низкий приоритет
          
          ШАГ 0.3: РАНЖИРОВАНИЕ И ФИЛЬТРАЦИЯ
            - Присвоить каждому результату оценку релевантности (0-100%)
            - Исключить результаты с запрещёнными брендами/категориями
            - Исключить сайты из списка исключений
            - Отсортировать по убыванию релевантности
            - Сформировать ПРИОРИТЕТНЫЙ СПИСОК для физических переходов
          
          ШАГ 0.4: ПРОВЕРКА ДОСТАТОЧНОСТИ ВЫДАЧИ
            ЕСЛИ на первой странице >= 5 результатов с релевантностью >= 50%:
              → Начать физические переходы по приоритетному списку
            
            ЕСЛИ на первой странице < 5 результатов с релевантностью >= 50%:
              → Перейти на ВТОРУЮ страницу выдачи Google:
                1. Найти элемент пагинации (кнопка "Следующая" или "2")
                2. Выполнить browser_click(element, ref)
                3. Дождаться загрузки через browser_wait_for(time: 2)
                4. Получить browser_snapshot() новой страницы
                5. Проанализировать новые результаты по тем же критериям
                6. Добавить релевантные в приоритетный список
            
            ЕСЛИ после двух страниц < 3 потенциально релевантных результатов:
              → Сформировать альтернативный поисковый запрос (см. search_strategy)
          
          ШАГ 0.5: ЛОГИРОВАНИЕ АНАЛИЗА ВЫДАЧИ
            Формат:
            === Анализ выдачи Google для: "[название товара]" ===
            Страница 1:
            1. [85%] https://krepeg.ru/vint-4216 — "Винт 4.2х16 DIN 7981" ✅ для перехода
            2. [72%] https://metiz.ru/product/123 — "Винты самонарезающие" ✅ для перехода
            3. [60%] https://bolt.ru/fasteners — "Каталог крепежа" ⚠️ страница категории
            4. [0%] https://hilti.ru/product — "Hilti винт" ⛔ запрещённый бренд
            5. [45%] https://forum.ru/topic — "Какие винты лучше?" ❌ низкая релевантность
            ...
            Итого релевантных: 4 | Для переходов: #1, #2, #6, #7
            === Начинаю физическую проверку по приоритету ===
        
        mandatory_source_validation_algorithm: |
          ВХОДНЫЕ ДАННЫЕ: приоритетный список URL из serp_analysis (отфильтрованный и ранжированный по релевантности)
          
          ⚠️ ВАЖНО: Переходить ТОЛЬКО по ссылкам из приоритетного списка, начиная с самых релевантных!
          Не переходить по ссылкам, помеченным как ⛔ (запрещённый бренд/категория) или ❌ (низкая релевантность).
          
          ДЛЯ КАЖДОГО URL ИЗ ПРИОРИТЕТНОГО СПИСКА (в порядке убывания релевантности) выполнить СТРОГО ПО ПОРЯДКУ:
          
          ШАГ 1: ПЕРЕХОД НА СТРАНИЦУ
            - Выполнить: browser_navigate(url)
            - Таймаут: 10 секунд
            - При ошибке/таймауте: записать в лог "⚠️ Ошибка загрузки", перейти к следующему URL из приоритетного списка
          ...
          ШАГ 6: ПРОВЕРКА ПРОГРЕССА И РАСШИРЕНИЕ ПОИСКА
            ЕСЛИ найдено 3 валидных источника:
              → Завершить поиск, перейти к финальному решению
            
            ЕСЛИ приоритетный список исчерпан И найдено < 3 источников:
              → Проверить, анализировалась ли вторая страница Google:
                - Если НЕТ → вернуться на Google, перейти на вторую страницу выдачи:
                  1. browser_navigate("назад на Google")
                  2. browser_click("кнопка Следующая или 2")
                  3. browser_snapshot() для анализа новых результатов
                  4. Выполнить serp_analysis для новой страницы
                  5. Продолжить переходы по обновлённому приоритетному списку
                - Если ДА → сформировать альтернативный поисковый запрос (до 3 попыток всего)
          
          ПОВТОРЯТЬ пока не найдено 3 валидных источника ИЛИ не исчерпаны все попытки (3 запроса × 2 страницы каждый).
...
serp_analysis_rules:
  description: Правила предварительного анализа поисковой выдачи Google с учётом критериев задачи
  
  purpose: |
    Цель предварительного анализа — НЕ принимать решения по сниппетам, а ОПТИМИЗИРОВАТЬ порядок переходов:
    - Сначала переходить на самые перспективные источники
    - Не тратить время на заведомо нерелевантные ссылки
    - Учитывать критерии из requirements_checklist ещё на этапе фильтрации
  
  relevance_scoring:
    high_relevance_signals (70-100%):
      - Заголовок содержит ТОЧНОЕ название товара из колонки A
      - URL содержит /product/, /tovar/, /item/, артикул или модель товара
      - Сниппет упоминает цену в рублях и "купить"/"в наличии"
      - Домен — известный специализированный магазин
    
    medium_relevance_signals (50-69%):
      - Заголовок содержит частичное совпадение (тип товара или бренд)
      - URL ведёт в каталог близкой категории
      - Сниппет описывает похожий товар
    
    low_relevance_signals (<50%):
      - Заголовок не связан с товаром
      - URL — главная страница, страница поиска или форум
      - Сниппет — статья, обзор, сравнение без цены
  
  criteria_based_filtering:
    description: Фильтрация на основе критериев из requirements_checklist
    
    brand_check (J):
      - Сканировать заголовок и сниппет на наличие брендов из brand_blacklist
      - При обнаружении: пометить "⛔ запрещённый бренд: [название]", исключить из переходов
      - Примеры: "Hilti", "Bosch", "DeWalt", "Apple", "Dell"
    
    category_check (K):
      - Сканировать на признаки категорий из category_blacklist
      - При обнаружении: пометить "⛔ запрещённая категория: [название]", исключить
      - Примеры: "автозапчасти", "стекло", "зеркала", "бензин"
    
    standardization_check (H):
      - Искать признаки нестандартного товара: "ОЛ", "ТЗ", "Черт.", "по чертежу", "индивидуальный"
      - При обнаружении: понизить приоритет, но НЕ исключать (требует проверки на странице)
    
    availability_check (I, L):
      - Искать признаки недоступности: "снят с производства", "discontinued", "под заказ", "нет в наличии"
      - При обнаружении: понизить приоритет, пометить для проверки критерия L
  
  pagination_rules:
    trigger: Менее 5 результатов с релевантностью >= 50% на первой странице
    max_pages: 2 (первая + одна дополнительная)
    action_sequence:
      1. Найти элемент пагинации внизу страницы Google
      2. Выполнить browser_click() на "Следующая" или номер "2"
      3. Дождаться загрузки (browser_wait_for(time: 2))
      4. Выполнить browser_snapshot() новой страницы
      5. Проанализировать результаты по тем же правилам
      6. Объединить с результатами первой страницы
      7. Пересортировать общий список по релевантности
  
  output_format: |
    === Анализ выдачи Google для: "[название товара из A]" ===
    Поисковый запрос: "[запрос]"
    
    Страница 1 (N результатов):
    1. [85%] https://krepeg.ru/vint-4216 — "Винт 4.2х16 DIN 7981 - Купить" ✅
    2. [72%] https://metiz.ru/product/123 — "Винты самонарезающие 4.2" ✅
    3. [0%] https://hilti.ru/product/456 — "Hilti винт" ⛔ запрещённый бренд
    4. [60%] https://bolt.ru/catalog/vinty — "Каталог винтов" ⚠️ страница категории
    5. [40%] https://forum.diy.ru/topic/789 — "Какие винты лучше для металла?" ❌ форум
    ...
    
    Страница 2 (если потребовалась):
    11. [78%] https://metizopt.ru/vint-din7981 — "Винт DIN 7981 4.2х16" ✅
    ...
    
    ИТОГО: Проанализировано [X] результатов | Релевантных >= 50%: [Y] | Для переходов: [Z]
    Приоритетный список: #1 → #2 → #11 → #6 → ...
    === Начинаю физическую проверку ===
...
behavior_examples:
  ...
  correct_behavior:
    ...
    - example: 6
      scenario: "Агент получает snapshot выдачи Google, видит 10 результатов, анализирует ВСЕ по заголовкам/сниппетам/URL, ранжирует по релевантности, исключает ссылку с 'Hilti' (запрещённый бренд), затем переходит по топ-5 в порядке убывания релевантности"
      marker: "✅ ПРАВИЛЬНО"
      why: "Предварительный анализ с учётом критериев задачи экономит время и повышает качество выбора источников"
    
    - example: 7
      scenario: "На первой странице Google только 3 результата с релевантностью >= 50%; агент переходит на вторую страницу выдачи через browser_click на пагинацию, анализирует новые результаты и находит ещё 4 релевантных ссылки"
      marker: "✅ ПРАВИЛЬНО"
      why: "Использование пагинации Google для расширения пула потенциальных источников при недостатке данных"
    
    - example: 8
      scenario: "Агент видит в сниппете 'под заказ, срок 14 дней' — не исключает ссылку, но помечает для проверки критерия L и понижает приоритет; после физического перехода подтверждает L='Да'"
      marker: "✅ ПРАВИЛЬНО"
      why: "Предварительный анализ информирует о потенциальных проблемах, но финальное решение — только после проверки страницы"
