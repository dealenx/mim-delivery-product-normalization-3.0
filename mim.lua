...
requirements_checklist:
  ...
  dimensions:
    column: R
    description: Негабарит.
    criteria: Крупногабаритный (> 1200x800x1600 мм).
    logic: Если крупный (негабарит) -> R="Да". Если нормальный -> R="Нет".
...
workflow:
    ...
    substeps:
      ...
      - name: marketplace_search
        ...
        actions:
          ...
          - ВАЖНО: Сохранять ссылку в E, F, G ТОЛЬКО если товар найден и соответствует запросу (схожесть >= 70%).
          - Если товар не найден или схожесть низкая -> Ссылку НЕ сохранять (оставить поле пустым).
          - Если после 3 попыток не найдено 3 подходящих источника -> D="Нет".
          - Заполнить E, F, G только валидными ссылками.
          - Проверить наличие (L).
  ...
  - step: 2
    ...
    logic: |
      ...
      2. I="Да" (Выпускается)
      3. Заполнены E, F, G (Найдено 3 ВАЛИДНЫХ источника с правильным товаром)
      ...
      Примеры для S: "Меньше 3 источников", "Товар под заказ", "Запрещенный бренд", "Сложный монтаж".
